					...
					if (characterID != null && state != null) {
						...
						for (Object supportMeta : suppportMetas) {
							if (supportMeta instanceof Map<?,?>) {
								...
								if (map.containsKey(NeXMLUtil.DC_DESCRIPTION_PREDICATE) && map.containsKey(NeXMLUtil.DC_SOURCE_PREDICATE)) {
									...
									final AssociationSupport associationSupport = new AssociationSupport(stringOrNull(NeXMLUtil.first(descriptionList)), stringOrNull(NeXMLUtil.first(sourceList)));
									final Association association = new Association(otuID, characterID, stateID);
